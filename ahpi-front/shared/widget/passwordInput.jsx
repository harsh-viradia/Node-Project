...
const PasswordInput = ({ placeholder, rest, error, disabled = false, label = false, mandatory = false, className }) => {
  ...
  return (
    <div className="text-left">
      ...
      <div className="flex items-center gap-3 relative">
        <input type={showPassword ? "text" : "password"} placeholder={placeholder} className={classes} {...rest} />

        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4">
          {showPassword ? <EyeCloseIcon /> : <EyeIcon fill="#888" />}
        </button>
      </div>
